...
const Select:React.FC<IProps> = ({ label, options, value, setValue, type, reset, name }) => {
    ...
    return (
        <div className='flex flex-col gap-2 flex-1'>
            ...
            <select
                value={reset ? '' : value}
                onChange={(e) => !name ? setValue(e.target.value) : setValue((prev:any) => ({ ...prev, [name]: e.target.value }))}
                id="select-address"
